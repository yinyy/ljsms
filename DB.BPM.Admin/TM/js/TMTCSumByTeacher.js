...
$(function () {
    ...
    $('#ToExcel').click(function () {
        ...
        var ee = new ExportExcel('list', "V_TM_TeachCheckSumByCourse");
        ee.go();
    });
                ...
                $.getJSON(actionURLS + '?' + createParam('classes', newValue), function (dd) {
                    ...
                    $('#txt_ClassID').combobox({
                        ...
                        onChange: function (nV, oV) {
                            ...
                            ruleArr.push({ "field": "TeacherID", "op": "eq", "data": uid });
                            if (ruleArr.length > 0) {
...
var grid = {
    bind: function (winSize) {
        $('#list').datagrid({
            url: actionURL + "?"+createParam("GetCourseSumByTeacher"),

            toolbar: '#toolbar',
            title: "我任教课程学生缺勤情况汇总表",
            ...
            columns: [[
             ...
             { title: '学时', field: 'CourseLearningTime', width: 40 },
               { title: '班级', field: 'ClassName', width: 140 },
            ...
            rowStyler: function (index, row) {
                if (row.CourseLearningTime == 0)
                {
                    return "";
                }
                if (row.CheckKK > row.CourseLearningTime / 6) {
                    return "background-color:red;color:yellow;font-weight:bold;";
                }
                else {
                    return "";
                }
