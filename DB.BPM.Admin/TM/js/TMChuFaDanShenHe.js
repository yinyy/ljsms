...
$(function () {
    ...
    $('#a_audit').click(CRUD.shenhe);
    $('#a_quxiaoshenhe').click(CRUD.quxiaoshenhe);
    $('#a_shehesuoyou').click(CRUD.shenheall);
    //高级查询
...
var CRUD = {
    ...
    shenhe: function () {
        ...
        if (row) {
            if (row.ShenHeYiJian != "未审批") {
                alert("本处罚单已经于[" + row.ShenHeDate + "]被[" + row.ShenHeRenName + "]审批通过，不得重复再做审批操作！");
                return false;
            }
            var hDialog = top.jQuery.hDialog({
                title: '罚分单审核', width: 650, height: 400, href: formurl, iconCls: 'icon-save',
                onLoad: function () {
                    //获取违纪地点字典并绑定
                    top.$("#txt_WeiJiAddress").combobox({
                        url: '/sys/ashx/DicHandler.ashx?categoryId=17',
                    ...
                    top.$("#txt_RuleId").combogrid(
              {
                ...
                submit: function () {
                    if (top.$('#uiform').validate().form()) {
                        var query =createParam('shenhe', row.KeyId);;
                        jQuery.ajaxjson(actionURL, query, function (d) {
                            if (parseInt(d) > 0) {
                                msg.ok('审批成功！');
                                hDialog.dialog('close');
            ...
            msg.warning('请选择要审核的数据行。');
        }
    },
    quxiaoshenhe: function () {
        var row = grid.getSelectedRow();
        if (row) {
            
            if (row.ShenHeYiJian == "未审批") {
                alert("本处罚单尚未审批，不需要取消审批！");
                return false;
            }
            
            if (confirm('确认要执行取消审批操作吗？')) {
                var rid = row.KeyId;
                jQuery.ajaxjson(actionURL, createParam('quxiaoshenhe', rid), function (d) {
                    if (parseInt(d) > 0) {
                        msg.ok('取消审批成功！');
                        grid.reload();
                    } else {
    ...
    shenheall: function () {
        ...
        if (confirm('确认要执行审批所有未审批的罚分单操作吗？本操作不可逆，请慎重执行！')) {            
            jQuery.ajaxjson(actionURL, createParam('shenheall'), function (d) {
                if (parseInt(d) > 0) {
                    msg.ok('审批成功！');
                    grid.reload();
                } else {
