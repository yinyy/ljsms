...
$(function () {
    ...
    $('#a_studentsum').click(sum.personsum);
    //高级查询
...
var sum = {
    datesum: function () {
        bindsum("datechufasum", "日期");
    },
    classsum: function () {
        bindsum("classchufasum", "班级");
    },
    personsum: function () {
        bindsum("personchufasum", "处罚人");
    }
};
var grid = {
...
function bindsum(durl, mbt) {
    ...
    var hDialog = top.jQuery.hDialog({
       ...
       // alert(d1+"----"+d2);
        if (d1 > d2) {
            alert("日期查询起止顺序有错误！请检查！");
            return;
        }
            pgrid = top.$('#sumlist').datagrid({
                url: actionURL + '?action=' + durl+'&sday='+d1+'&eday='+d2,
                width: 550,
                pagination: true,
                pageSize: PAGESIZE,
                pageList: [200],
