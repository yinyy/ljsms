...
namespace DB.BPM.Core.Dal
{
    public class UserDal : BaseRepository<User>
    {
        ...
        public IEnumerable<User> GetUsersByDepartmentID(int depid)
        {
            return base.GetAll().Where(n => n.DepartmentId== depid);
        }
        /// <summary>
        /// 为指定的用户分配角色
        /// </summary>
        ...
        public List<User> GetUsersByrid(int roleid)
        {
            string sql = "select * from Sys_Users where KeyId in (select UserID from Sys_UserRoles where roleid=@Roleid)";
            return DbUtils.ExecuteReader<User>(sql, new { Roleid = roleid }).ToList();
        }
